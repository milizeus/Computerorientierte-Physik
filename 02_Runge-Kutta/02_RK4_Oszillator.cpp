...
#define T_MAX       30 //*M_PI                              // max for t
...
double  F_ODE(double time, double y[], int i) {
    if (i==0) return( y[1]);                                                               // dY[0]/dt
    if (i==1) return( - damping*y[1] - omega*omega*y[1] + amp*sin(Omega * time)-y[0] );    // dY[1]/dt
}
...
int main(void){
    ...
    ofstream dataoutput("02_RK4_Oszillator.dat");
